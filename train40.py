...
def train_color_40(pretrained=False):
    ...
    if pretrained == False:
        ...
        ckpt_id = load_last_ckpt('color40')
        print('load pretrained model at ckpt_{}'.format(ckpt_id))
        model.load_state_dict(torch.load('./log/color40/' + 'ckpt_' + str(ckpt_id) + '.pkl'))
        ckpt_id += 1
    ...
    for epoch in range(config.num_epochs):
        for i, (images, labels) in enumerate(imageHelper.train_loader):
            ...
            noise_images= imageHelper.get_noise_batch(true_images,0.4,config.batch_size).to(device)
            ...
            if (i + 1) % 100 == 0:
                ...
                cv2.imwrite('./output/color40/epoch'+ str(epoch+1)+'_step'+ str(i+1)+'_output.png', tempimage*255)
                trueimage = true_images[0].cpu().numpy()
                trueimage = np.transpose(trueimage, (1, 2, 0))
                cv2.imwrite('./output/color40/epoch' + str(epoch + 1) + '_step' + str(i + 1) + '_true.png', trueimage * 255)
                noiseimage = noise_images[0].cpu().numpy()
                noiseimage = np.transpose(noiseimage, (1, 2, 0))
                cv2.imwrite('./output/color40/epoch' + str(epoch + 1) + '_step' + str(i + 1) + '_noise.png', noiseimage * 255)

        print('save ckpt\n')
        torch.save(model.state_dict(), './log/color40/' + 'ckpt_' + str(epoch + ckpt_id) + '.pkl')
        # val_color_40(epoch + ckpt_id)
...
def val_color_40():
    ...
    for i, (images, labels) in enumerate(imageHelper.test_loader):
        ...
        if (i + 1) % 50 == 0:
            ...
            tempimage = outputs[0].detach().cpu().numpy()  # numpy
            tempimage = np.transpose(tempimage, (1, 2, 0))
            cv2.imwrite('./output/color40/val'  + '_step' + str(i + 1) + '_output.png',
                        tempimage * 255)
            trueimage = true_images[0].cpu().numpy()
            trueimage = np.transpose(trueimage, (1, 2, 0))
            cv2.imwrite('./output/color40/val' + '_step' + str(i + 1) + '_true.png', trueimage * 255)
            noiseimage = noise_images[0].cpu().numpy()
            noiseimage = np.transpose(noiseimage, (1, 2, 0))
            cv2.imwrite('./output/color40/val' + '_step' + str(i + 1) + '_noise.png',
                        noiseimage * 255)



if __name__ == '__main__':
    train_color_40(pretrained=True)
    # val_color_40()
    # test_color_40("B.png")
